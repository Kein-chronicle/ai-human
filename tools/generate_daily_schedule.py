...
EVENING_POOL = [
    {"activity": "독서", "detail": "법률 관련 서적 or 소설"},
    {"activity": "유튜브 / OTT", "detail": "넷플릭스 드라마"},
    {"activity": "스트레칭 + 샤워", "detail": "집에서 가볍게"},
    {"activity": "런닝", "detail": "한강 or 동네 30분"},
    {"activity": "피트니스", "detail": "헬스장 1시간"},
    {"activity": "영어 공부", "detail": "법률영어 인강 or 단어"},
    {"activity": "음악 감상", "detail": "블루투스 스피커로 좋아하는 플레이리스트"},
    {"activity": "카톡 + 유튜브", "detail": "그냥 쉬기"},
    {"activity": "셀프 케어", "detail": "마스크팩 + 스킨케어"},
]
...
def generate_weekday(date_str: str) -> dict:
    ...
    if dinner["cook"] == "배달":
        dinner_cook_start = hm(arrive_home, 5)
        dinner_eat = hm(arrive_home, 35)
    else:
        dinner_cook_start = hm(arrive_home, 15)
        dinner_eat = hm(dinner_cook_start, random.randint(20, 30))
    dinner_end = hm(dinner_eat, 25)

    evening = random.choice(EVENING_POOL)
    exercise_start = hm(dinner_end, 15)
    exercise_end = hm(exercise_start, evening.get("duration_min", 60)) if evening.get("duration_min", 0) > 0 else exercise_start
    if evening.get("shower", False):
        ...
        shower_after = None
        hair_dry = None
        night_start = exercise_end if evening.get("duration_min", 0) > 0 else hm(dinner_end, 30)
    ...
    if st_m <= ns_m:
        st_m = ns_m + 90
    night_timeline = [
        {"time": f"{ns_m // 60:02d}:{ns_m % 60:02d}", "activity": "스킨케어 하고 편한 옷으로 갈아입기"},
        {"time": f"{(ns_m + 15) // 60:02d}:{(ns_m + 15) % 60:02d}", "activity": evening.get("night_content", "OTT/독서 등")},
        {"time": f"{max(ns_m + 45, st_m - 30) // 60:02d}:{max(ns_m + 45, st_m - 30) % 60:02d}", "activity": "마무리 루틴"},
        {"time": sleep, "activity": "잠자리에 눕기"},
    ]
    ...
    return {
        ...
        "morning": {
            ...
            "outfit": random.choice([
                "정장 (네이비 블레이저 + 슬랙스)", "정장 (화이트 블라우스 + 슬랙스)",
                "세미정장 (니트 + 슬랙스)", "블라우스 + 펜슬 스커트",
                "셔츠 + 슬랙스", "재킷 + 드레스 팬츠",
            ]),
        ...
        "evening": {
            "activity_type": evening["activity"],
            "detail": evening.get("detail", ""),
            "start_time": exercise_start,
            "duration_min": evening.get("duration_min", 0),
