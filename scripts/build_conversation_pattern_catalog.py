#!/usr/bin/env python3
"""
Builds conversation_pattern_catalog.json from messages/*.jsonl and approved patterns.
Customize PATTERN_RULES and filter lists per character before use.
"""
import json
import re
import sys
...
def text_length_profile(texts: list) -> dict:
    ...
    question_ratio = round(sum(1 for text in texts if "?" in text or "까" in text[-3:]) / len(texts), 3)
    return {
...
def build_catalog():
    ...
    return {
        ...
        "global_length_profile": {
            "character_outgoing": text_length_profile(outgoing),
